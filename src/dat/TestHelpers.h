...
#define COMPARE_MEMBER(MEMBER) \
    if (this->MEMBER != other.MEMBER) { \
        std::cout << #MEMBER " differs in " << __FILE__ << std::endl; \
        return false; \
    }
...
#define COMPARE_MEMBER_VEC(MEMBER) \
    if (MEMBER.size() != other.MEMBER.size()) { \
        std::cout << "this " << MEMBER.size() << " size diff vs " << other.MEMBER.size() << " at " << __FILE__ << std::endl; \
        return false; \
    } \
    for (size_t i=0; i<MEMBER.size(); i++) { \
        if (!MEMBER[i].compareTo(other.MEMBER[i])) { \
            std::cout << #MEMBER " vector differs in " << __FILE__ << std::endl; \
            return false; \
        } \
    }
