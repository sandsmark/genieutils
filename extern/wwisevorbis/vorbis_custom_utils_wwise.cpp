...
static size_t get_packet_header(std::istream *streamFile, off_t offset, wwise_header_t header_type, int * granulepos, size_t * packet_size, int big_endian);
static size_t rebuild_packet(uint8_t * obuf, size_t obufsize, std::istream *streamFile, off_t offset, vorbis_custom_codec_data * data, int big_endian);
static size_t rebuild_setup(uint8_t * obuf, size_t obufsize, std::istream *streamFile, off_t offset, vorbis_custom_codec_data * data, int big_endian, int channels);

static int ww2ogg_generate_vorbis_packet(vgm_bitstream * ow, vgm_bitstream * iw, std::istream *streamFile, off_t offset, vorbis_custom_codec_data * data, int big_endian);
static int ww2ogg_generate_vorbis_setup(vgm_bitstream * ow, vgm_bitstream * iw, vorbis_custom_codec_data * data, int channels, size_t packet_size, std::istream *streamFile);
static int ww2ogg_codebook_library_copy(vgm_bitstream * ow, vgm_bitstream * iw);
static int ww2ogg_codebook_library_rebuild(vgm_bitstream * ow, vgm_bitstream * iw, size_t cb_size, std::istream *streamFile);
static int ww2ogg_codebook_library_rebuild_by_id(vgm_bitstream * ow, uint32_t codebook_id, wwise_setup_t setup_type, std::istream *streamFile);
...
static int load_wvc(uint8_t * ibuf, size_t ibufsize, uint32_t codebook_id, wwise_setup_t setup_type, std::istream *streamFile);
static int load_wvc_file(uint8_t * buf, size_t bufsize, uint32_t codebook_id, std::istream *streamFile);
...
static size_t get_packet_header(std::istream *streamFile, off_t offset, wwise_header_t header_type, int * granulepos, size_t * packet_size, int big_endian) {
//    int32_t (*read_32bit)(off_t,std::istream*) = big_endian ? read_32bitBE : read_32bitLE;
//    int16_t (*read_16bit)(off_t,std::istream*) = big_endian ? read_16bitBE : read_16bitLE;

    /* packet size doesn't include header size */
    switch(header_type) {
        case WWV_TYPE_8: /* size 4+4 */
            streamFile->seekg(offset);
            streamFile->read((char*)packet_size, sizeof(uint32_t));
            streamFile->seekg(offset+4);
            streamFile->read((char*)granulepos, sizeof(uint32_t));
            return 8;

        case WWV_TYPE_6: /* size 4+2 */
            streamFile->seekg(offset);
            streamFile->read((char*)packet_size, sizeof(uint16_t));
            streamFile->seekg(offset+2);
            streamFile->read((char*)granulepos, sizeof(uint32_t));
            return 6;
...
static size_t rebuild_packet(uint8_t * obuf, size_t obufsize, std::istream *streamFile, off_t offset, vorbis_custom_codec_data * data, int big_endian) {
    ...
    streamFile->seekg(offset+header_size);
    streamFile->read((char*)ibuf, packet_size);
...
//        goto fail;
    ...
    if (ow.b_off % 8 != 0) {
        //printf("Wwise Vorbis: didn't write exactly audio packet: 0x%lx + %li bits\n", ow.b_off / 8, ow.b_off % 8);
        goto fail;
    }
...
//        goto fail;
    ...
    if (ow.b_off % 8 != 0) {
        //printf("Wwise Vorbis: didn't write exactly setup packet: 0x%lx + %li bits\n", ow.b_off / 8, ow.b_off % 8);
        goto fail;
    }
...
static int ww2ogg_generate_vorbis_packet(vgm_bitstream * ow, vgm_bitstream * iw, std::istream *streamFile, off_t offset, vorbis_custom_codec_data * data, int big_endian) {
    ...
    std::streampos currpos = streamFile->tellg();
    ...
    streamFile->seekg(std::ios::end);
    data_size = streamFile->tellg();
    streamFile->seekg(currpos);
//    data_size = get_streamfile_size(streamFile);//todo get external data_size

    if (offset + header_size + packet_size > data_size) {
        printf("Wwise Vorbis: page header truncated\n");
        goto fail;
    }
    ...
    if (data->config.packet_type == WWV_MODIFIED) {
        ...
        if (!data->mode_blockflag) { /* config error */
            printf("Wwise Vorbis: didn't load mode_blockflag\n");
            goto fail;
        }
        ...
        if (data->mode_blockflag[mode_number]) {
            ...
            if (next_offset + header_size <= data_size) {
                ...
                if (next_packet_size > 0) {
                    ...
                    streamFile->seekg(next_offset + next_header_size);
                    streamFile->read((char*)nw.buf, nw.bufsize);
...
static int ww2ogg_generate_vorbis_setup(vgm_bitstream * ow, vgm_bitstream * iw, vorbis_custom_codec_data * data, int channels, size_t packet_size, std::istream *streamFile) {
    ...
    if (data->config.setup_type == WWV_FULL_SETUP) {
            ...
            for (j = 0; j <= maximum_class; j++) {
                ...
                if (0 != class_subclasses) {
                    ...
                    if (masterbook >= codebook_count) {
                        printf("Wwise Vorbis: invalid floor1 masterbook\n");
                        goto fail;
                    }
                }
                ...
                for (k = 0; k < (1U<<class_subclasses); k++) {
                    ...
                    if (subclass_book >= 0 && subclass_book >= codebook_count) {
                        printf("Wwise Vorbis: invalid floor1 subclass book\n");
                        goto fail;
                    }
                }
            }
        ...
        for (i = 0; i < residue_count; i++) {
            ...
            if (residue_type > 2) {
                printf("Wwise Vorbis: invalid residue type\n");
                goto fail;
            }
            ...
            if (residue_classbook >= codebook_count) {
                printf("Wwise Vorbis: invalid residue classbook\n");
                goto fail;
            }
                ...
                for (k = 0; k < 8; k++) {
                    if (residue_cascade[j] & (1 << k)) {
                        ...
                        if (residue_book >= codebook_count) {
                            printf("Wwise Vorbis: invalid residue book\n");
                            goto fail;
                        }
                    }
                }
        ...
        for (i = 0; i < mapping_count; i++) {
            ...
            if (square_polar_flag) {
                ...
                for (j = 0; j < coupling_steps; j++) {
                    ...
                    if (angle == magnitude || magnitude >= channels || angle >= channels) {
                        printf("Wwise Vorbis: invalid coupling (angle=%i, mag=%i, ch=%i)\n", angle, magnitude,channels);
                        goto fail;
                    }
                }
            }
            ...
            if (0 != mapping_reserved) {
                printf("Wwise Vorbis: mapping reserved field nonzero\n");
                goto fail;
            }
            ...
            if (submaps > 1) {
                for (j = 0; j < channels; j++) {
                    ...
                    if (mapping_mux >= submaps) {
                        printf("Wwise Vorbis: mapping_mux >= submaps\n");
                        goto fail;
                    }
                }
            }
            ...
            for (j = 0; j < submaps; j++) {
                ...
                if (floor_number >= floor_count) {
                    printf("Wwise Vorbis: invalid floor mapping\n");
                    goto fail;
                }
                ...
                if (residue_number >= residue_count) {
                    printf("Wwise Vorbis: invalid residue mapping\n");
                    goto fail;
                }
            }
        }
        ...
        for (i = 0; i < mode_count; i++) {
            ...
            if (mapping >= mapping_count) {
                printf("Wwise Vorbis: invalid mode mapping\n");
                goto fail;
            }
        }
    }
...
static int ww2ogg_codebook_library_copy(vgm_bitstream * ow, vgm_bitstream * iw) {
    ...
    if (0x564342 != id) { /* "VCB" */
        printf("Wwise Vorbis: invalid codebook identifier\n");
        goto fail;
    }
    ...
    if (ordered) {
        ...
        if (current_entry > entries) {
            printf("Wwise Vorbis: current_entry out of range\n");
            goto fail;
        }
    }
    else {
    ...
    if (0 == lookup_type) {
        //printf("Wwise Vorbis: no lookup table\n");
    }
    else if (1 == lookup_type) {
        //printf("Wwise Vorbis: lookup type 1\n");
        ...
        printf("Wwise Vorbis: didn't expect lookup type 2\n");
        goto fail;
    }
    else {
        printf("Wwise Vorbis: invalid lookup type\n");
        goto fail;
    }
...
static int ww2ogg_codebook_library_rebuild(vgm_bitstream * ow, vgm_bitstream * iw, size_t cb_size, std::istream *streamFile) {
    ...
    if (ordered) {
        ...
        if (0 == codeword_length_length || 5 < codeword_length_length) {
            printf("Wwise Vorbis: nonsense codeword length\n");
            goto fail;
        }
    ...
    if ( 0 != cb_size && iw->b_off/8+1 != cb_size ) {
        //printf("Wwise Vorbis: codebook size mistach (expected 0x%x, wrote 0x%lx)\n", cb_size, iw->b_off/8+1);
        goto fail;
    }
